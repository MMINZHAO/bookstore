    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-tooltip class="item" effect="light" content="支付" placement="bottom-end">
            <el-button size="mini" type="primary" @click="handleEdit(scope.row)"><i class="el-icon-success"></i></el-button>
          </el-tooltip>
          <el-tooltip class="item" effect="light" content="删除订单" placement="bottom-end">
            <el-button  size="mini" type="danger" @click="handleDel(scope.row)"><i class="el-icon-delete"></i></el-button>
          </el-tooltip>
  ...
  export default {
    ...
    methods:{
      ...
      handleEdit(val){
        ...
        if(val.payment != ''){
          this.$message.error('该订单已支付！');
        }
        else
        bus.$emit("payItem",{flag:true,value:val.order_num})
      },
      handleDel(val){   //删除一条记录
        if(val.payment != ''){
          this.$message.error('已支付订单不允许删除！');
        }
        else
        this.$confirm('删除该订单？, 是否继续?', '提示',{
          ...
          params.append("id",val.order_num);
          this.axios.post('http://localhost:8200/order/delOrder',params)
            .then( res => {
              this.$message.success('移除成功！');
              //删除成功后重新请求当前页
              this.axios.get('http://localhost:8200/order/getMyOrder',{
                params: {
